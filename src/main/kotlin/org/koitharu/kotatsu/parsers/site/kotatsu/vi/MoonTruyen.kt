...
@MangaSourceParser("MOONTRUYEN", "MoonTruyen", "vi", type = ContentType.HENTAI)
internal class MoonTruyen(context: MangaLoaderContext) :
	PagedMangaParser(context, MangaParserSource.MOONTRUYEN, 20) {

	override val configKeyDomain = ConfigKey.Domain("moontruyen.com")
	...
	override suspend fun getDetails(manga: Manga): Manga {
		...
		val description = doc.selectFirst("div.story-detail-info p")?.text()
		...
		val chapters = doc.select("div.chapter-list a").mapNotNull { a ->
			val href = a.absUrl("href").toRelativeUrl(domain)
			val name = a.selectFirst(".chapter-name")?.text() ?: "Chapter"
			val dateStr = a.selectFirst(".time")?.text().orEmpty()
			MangaChapter(
				id = generateUid(href),
				title = name,
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		...
		// Try to extract encrypted images (CryptoAES)
		...
		if (cryptoKey != null && encryptedData != null) {
			try {
				val decrypted = decryptAES(encryptedData, cryptoKey)
				val json = JSONArray(decrypted)
				return json.mapToPages()
	...
	private fun JSONArray.mapToPages(): List<MangaPage> {
	...
	private fun decryptAES(data: String, key: String): String {
