...
@MangaSourceParser("NETTRUYENVIE", "NetTruyenViet", "vi")
internal class NetTruyenViet(context: MangaLoaderContext) :
	PagedMangaParser(context, MangaParserSource.NETTRUYENVIE, 20) {

	override val configKeyDomain = ConfigKey.Domain("nettruyenviet10.com")
	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		...
		val url = if (!query.isNullOrEmpty()) {
			...
			when (order) {
				SortOrder.POPULARITY -> "https://$domain/truyen-yeu-thich/page-$page/"
				SortOrder.NEWEST -> "https://$domain/truyen-moi/page-$page/"
				else -> "https://$domain/dang-cap-nhat/page-$page/"
			}
		}
		val doc = webClient.httpGet(url).parseHtml()
		return doc.select("div.book_list .book_avatar, .list-truyen-item-wrap").mapNotNull { element ->
